...
function installfaust {
	...
	$SUDO apt-get install --yes unzip libmicrohttpd-dev puredata-dev emacs-nox

	# Install all the needed SDK
	$SUDO apt-get install -y libgtk2.0-dev libasound2-dev libqrencode-dev portaudio19-dev
	$SUDO apt-get install -y libjack-jackd2-dev #qjackctl
	...
	if [ -n "$COMPILE_FAUST" ]; then
	    ...
	    $SUDO apt-get install --yes faust
	fi
